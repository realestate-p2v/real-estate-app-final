...
function buildCaptionFilters(
  ...
  const { outW, outH, branding, inputs, slotDuration } = ctx;
  ...
  switch (caption.kind) {
    ...
    case "address_line": {
      ...
      lines.forEach((ln, i) => {
        ...
        txtFilters.push(
          `drawtext=text='${escapeDrawtext(ln)}':` +
...
function buildSlotFragment(
  ...
  if (slot?.caption) {
    const captionFilters = buildCaptionFilters(
      slot.caption,
      { outW, outH, branding, inputs, slotDuration: effectiveDuration },
      clip.label
    );
