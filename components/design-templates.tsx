        ...
        <div className="absolute inset-x-0 bottom-0" style={{ height: `${100 - photoPercent}%`, backgroundColor: barColor }}>
          ...
          <div className="absolute inset-0" style={{ display: "flex", padding: `${barPadY}px ${barPadRight}px ${barPadY}px ${barPadLeft}px`, gap: Math.round(28 * unit) }}>
            ...
            <div style={{ flex: 1, textAlign: "right", minWidth: 0, display: "flex", flexDirection: "column", justifyContent: "center" }}>
              ...
              <p style={{ fontSize: priceFontSize, fontWeight: 800, color: accent, lineHeight: 1.0, margin: 0, letterSpacing: "-0.01em", textShadow: accentColor && !barLight ? `0 ${Math.round(2 * unit)}px ${Math.round(12 * unit)}px ${hexToRgba(accentColor, 0.3)}` : "none" }}>{priceText}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    return (
      <div className="relative overflow-hidden" style={{ width: w, height: h, fontFamily }}>
        ...
        <div className="absolute inset-x-0 bottom-0" style={{ height: `${100 - photoPercent}%`, backgroundColor: barColor }}>
          ...
          {logo && (
            <img src={logo} alt="Logo" className="absolute object-contain" style={{ bottom: Math.round(20 * unit), right: barPadX, maxWidth: Math.round(barH * 0.34), maxHeight: Math.round(barH * 0.18), opacity: 1 }} />
          )}
        </div>
      </div>
    );
  ...
  return (
    <div className="relative overflow-hidden" style={{ width: w, height: h, fontFamily }}>
      ...
      <div className="absolute inset-x-0 bottom-0" style={{ height: `${100 - photoPercent}%`, backgroundColor: barColor }}>
        ...
        {logo && (
          <img src={logo} alt="Logo" className="absolute object-contain" style={{ bottom: Math.round(20 * unit), right: barPadX, maxWidth: Math.round(barH * 0.30), maxHeight: Math.round(barH * 0.16), opacity: 1 }} />
        )}
      </div>
    </div>
  );
...
export function OpenHouseTemplate({ size, listingPhoto, videoElement, headshot, logo, address, beds, baths, sqft, price, date, time, agentName, phone, brokerage, fontFamily, barColor, accentColor }: {
  ...
  return (
    <div className="relative overflow-hidden" style={{ width: w, height: h, fontFamily }}>
      ...
      <div className="absolute inset-0" style={{ backgroundImage: "linear-gradient(to bottom, rgba(0,0,0,0.45) 0%, rgba(0,0,0,0.15) 25%, transparent 40%)" }} />
      <div className="absolute inset-0" style={{ backgroundImage: "linear-gradient(to top, rgba(0,0,0,0.65) 0%, rgba(0,0,0,0.35) 20%, transparent 45%)" }} />
      <div className="absolute inset-0" style={{ backgroundImage: "radial-gradient(ellipse at center, transparent 50%, rgba(0,0,0,0.20) 100%)" }} />
      <div className="absolute inset-x-0 top-0 flex flex-col items-center justify-center text-center" style={{ height: isStory ? "26%" : "34%", padding: `0 ${pad}px` }}>
        <div style={{ display: "inline-flex", alignItems: "center", padding: `${badgePadY}px ${badgePadX}px`, backgroundColor: badgeBg, borderRadius: Math.round(6 * unit), boxShadow: `0 ${Math.round(4 * unit)}px ${Math.round(24 * unit)}px ${hexToRgba(badgeBg, 0.4)}` }}>
          <span style={{ fontSize: badgeFontSize, fontWeight: 800, color: isLightColor(badgeBg) ? "#111" : "#fff", letterSpacing: "0.16em", textTransform: "uppercase" as const, lineHeight: 1 }}>Open House</span>
        </div>
        <p style={{ fontSize: dateFontSize, fontWeight: 800, color: "#ffffff", margin: 0, marginTop: Math.round(18 * unit), textShadow, letterSpacing: "0.02em" }}>{dateText}</p>
        <p style={{ fontSize: timeFontSize, fontWeight: 500, color: "rgba(255,255,255,0.75)", margin: 0, marginTop: Math.round(6 * unit), textShadow, letterSpacing: "0.03em" }}>{timeText}</p>
      </div>
      <div className="absolute inset-x-0 bottom-0" style={{ padding: `0 ${pad}px` }}>
        <div style={{ textAlign: "center", marginBottom: Math.round(14 * unit) }}>
          <div style={{ width: Math.round(60 * unit), height: Math.round(1.5 * unit), backgroundColor: "rgba(255,255,255,0.25)", margin: `0 auto ${Math.round(16 * unit)}px`, borderRadius: 1 }} />
          <p style={{ fontSize: addressFontSize, fontWeight: 700, color: "#ffffff", lineHeight: 1.2, margin: 0, textShadow, overflowWrap: "break-word" }}>{addressText}</p>
          <p style={{ fontSize: detailsFontSz, fontWeight: 500, color: "rgba(255,255,255,0.70)", margin: 0, marginTop: Math.round(6 * unit), letterSpacing: "0.04em", textShadow }}>{detailsText}</p>
          ...
          <p style={{ fontSize: priceFontSize, fontWeight: 800, color: accent, lineHeight: 1.0, margin: 0, letterSpacing: "-0.01em", textShadow: accentColor ? `0 ${Math.round(2 * unit)}px ${Math.round(14 * unit)}px ${hexToRgba(accentColor, 0.35)}` : textShadow }}>{priceText}</p>
        </div>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: Math.round(14 * unit), height: agentBarH, padding: `0 ${Math.round(24 * unit)}px`, backgroundColor: hexToRgba(barColor, 0.88), borderRadius: `${agentBarRadius}px ${agentBarRadius}px 0 0`, borderTop: `${Math.round(1.5 * unit)}px solid ${barLight ? "rgba(0,0,0,0.08)" : "rgba(255,255,255,0.10)"}` }}>
          {headshot ? (
            <img src={headshot} alt="Agent" style={{ width: headshotSz, height: headshotSz, borderRadius: "50%", objectFit: "cover", flexShrink: 0, border: `${Math.round(2.5 * unit)}px solid ${accentColor ? hexToRgba(accentColor, 0.5) : "rgba(255,255,255,0.25)"}` }} />
          ) : (
            <div style={{ width: headshotSz, height: headshotSz, borderRadius: "50%", backgroundColor: barLight ? "rgba(0,0,0,0.06)" : "rgba(255,255,255,0.06)", border: `${Math.round(2.5 * unit)}px solid ${barLight ? "rgba(0,0,0,0.10)" : "rgba(255,255,255,0.12)"}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <User style={{ width: headshotSz * 0.38, height: headshotSz * 0.38, color: barLight ? "rgba(0,0,0,0.25)" : "rgba(255,255,255,0.25)" }} />
            </div>
          )}
          <div style={{ minWidth: 0 }}>
            <p style={{ fontSize: agentFontSize, fontWeight: 700, color: barLight ? "#111827" : "#ffffff", margin: 0, whiteSpace: "nowrap" }}>{agentNameText}</p>
            <p style={{ fontSize: contactFontSize, fontWeight: 500, color: barLight ? "rgba(17,24,39,0.50)" : "rgba(255,255,255,0.50)", margin: 0, marginTop: Math.round(2 * unit), overflowWrap: "break-word" }}>{contactLine}</p>
          </div>
          {logo && (
            <img src={logo} alt="Logo" style={{ maxWidth: logoMaxW, maxHeight: logoMaxH, objectFit: "contain" as const, flexShrink: 0, marginLeft: "auto" }} />
          )}
        </div>
      </div>
    </div>
...
export function YardSignSplitBar({ width, height, headshot, logo, agentName, phone, email, brokerage, officeName, officePhone, headerText, topColor, bottomColor, fontFamily, qrDataUrl, bulletPoints }: {
  ...
  return (
    <div style={{ width, height, fontFamily, display: "flex", flexDirection: "column" }}>
      <div style={{ height: topH, backgroundColor: topColor, display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
        <div style={{ display: "flex", alignItems: "center", gap: Math.round(width * 0.03) }}>
          ...
          <p style={{ fontSize: headerSz, fontWeight: 900, color: topTextColor, letterSpacing: "0.14em", textTransform: "uppercase" as const, margin: 0, lineHeight: 1 }}>{headerLabel}</p>
          <div style={{ width: Math.round(width * 0.07), height: Math.round(3 * u), backgroundColor: topRuleColor }} />
        </div>
      </div>
      <div style={{ height: centerH, backgroundColor: "#ffffff", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: `${Math.round(centerH * 0.06)}px ${Math.round(width * 0.08)}px`, gap: Math.round(centerH * 0.03), position: "relative" }}>
        ...
        {headshot ? (
          <img src={headshot} alt="Agent" style={{ width: headshotSz, height: headshotSz, objectFit: "cover", borderRadius: "50%", border: `${Math.round(6 * u)}px solid ${topColor}`, boxShadow: `0 ${Math.round(8 * u)}px ${Math.round(28 * u)}px rgba(0,0,0,0.15)` }} />
        ) : (
          <div style={{ width: headshotSz, height: headshotSz, backgroundColor: "#f3f4f6", borderRadius: "50%", border: `${Math.round(6 * u)}px solid ${topColor}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <User style={{ width: headshotSz * 0.35, height: headshotSz * 0.35, color: "#9ca3af" }} />
          </div>
        )}
        <p style={{ fontSize: nameSz, fontWeight: 800, color: "#111", lineHeight: 1.1, margin: 0, textAlign: "center", letterSpacing: "0.05em", textTransform: "uppercase" as const, whiteSpace: "nowrap" }}>{nameText}</p>
        <div style={{ width: Math.round(width * 0.10), height: Math.round(4 * u), backgroundColor: topColor, borderRadius: 2 }} />
        <p style={{ fontSize: phoneSz, fontWeight: 600, color: "#333", margin: 0, textAlign: "center", letterSpacing: "0.04em" }}>{phoneText}</p>
        {email && <p style={{ fontSize: detailSz, fontWeight: 400, color: "#666", margin: 0, textAlign: "center", overflowWrap: "break-word" }}>{email}</p>}
        ...
        {(logo || qrDataUrl) && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: Math.round(width * 0.03), marginTop: Math.round(centerH * 0.01) }}>
            {logo && <img src={logo} alt="Logo" style={{ maxHeight: Math.round(centerH * 0.12), maxWidth: Math.round(width * 0.25), objectFit: "contain" as const }} />}
            {qrDataUrl && <img src={qrDataUrl} alt="QR" style={{ width: Math.round(centerH * 0.22), height: Math.round(centerH * 0.22), borderRadius: Math.round(4 * u) }} />}
          </div>
        )}
        ...
        <div style={{ textAlign: "center" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: Math.round(width * 0.025) }}>
            ...
            <div>
              {showBothLines ? (
                <>
                  <p style={{ fontSize: bottomNameSz, fontWeight: 800, color: bottomTextColor, margin: 0, letterSpacing: "0.05em", overflowWrap: "break-word" }}>{brokerage}</p>
                  <p style={{ fontSize: Math.round(bottomNameSz * 0.7), fontWeight: 600, color: bottomTextColor, margin: 0, marginTop: Math.round(height * 0.004), letterSpacing: "0.03em", opacity: 0.8, overflowWrap: "break-word" }}>{officeName}</p>
                </>
              ) : (
                <p style={{ fontSize: bottomNameSz, fontWeight: 800, color: bottomTextColor, margin: 0, letterSpacing: "0.05em", overflowWrap: "break-word" }}>{officeText}</p>
              )}
            </div>
            <div style={{ width: Math.round(width * 0.05), height: Math.round(2 * u), backgroundColor: bottomRuleColor }} />
          </div>
          {officePhone && <p style={{ fontSize: bottomPhoneSz, fontWeight: 600, color: bottomTextColor, margin: 0, marginTop: Math.round(height * 0.008), letterSpacing: "0.03em" }}>{officePhone}</p>}
        </div>
      </div>
    </div>
  );
...
export function YardSignSidebar({ width, height, headshot, logo, agentName, phone, email, brokerage, website, headerText, sidebarColor, mainBgColor, fontFamily, qrDataUrl, bulletPoints }: {
  ...
  return (
    <div style={{ width, height, fontFamily, display: "flex" }}>
      <div style={{ width: sideW, height, background: `linear-gradient(to bottom, ${sidebarColor}, ${darken(sidebarColor, 12)})`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "space-between", padding: `${Math.round(height * 0.04)}px ${Math.round(sideW * 0.08)}px` }}>
        {logo && <img src={logo} alt="Logo" style={{ width: logoSz, height: logoSz, objectFit: "contain" as const }} />}
        <p style={{ fontSize: Math.round(sideW * 0.20), fontWeight: 800, color: sideTextColor, writingMode: "vertical-rl" as any, textOrientation: "mixed" as any, letterSpacing: "0.14em", textTransform: "uppercase" as const, opacity: 0.9 }}>{brokerageText}</p>
        {logo && <img src={logo} alt="Logo" style={{ width: logoSz, height: logoSz, objectFit: "contain" as const }} />}
        {!logo && <div style={{ width: Math.round(sideW * 0.4), height: Math.round(2 * u), backgroundColor: sideMuted }} />}
      </div>
      <div style={{ width: mainW, height, backgroundColor: mainBgColor, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: `${Math.round(height * 0.02)}px ${Math.round(width * 0.04)}px`, textAlign: "center", position: "relative", gap: Math.round(height * 0.008) }}>
        <div style={{ position: "absolute", top: Math.round(height * 0.06), bottom: Math.round(height * 0.06), left: 0, width: Math.round(4 * u), backgroundColor: sidebarColor, opacity: 0.15 }} />
        <div style={{ backgroundColor: sidebarColor, padding: `${Math.round(height * 0.012)}px ${Math.round(width * 0.06)}px`, borderRadius: Math.round(6 * u), marginBottom: Math.round(height * 0.012) }}>
          <p style={{ fontSize: headerSz, fontWeight: 900, color: sideLight ? "#000" : "#fff", letterSpacing: "0.12em", textTransform: "uppercase" as const, margin: 0, lineHeight: 1 }}>{headerLabel}</p>
        </div>
        {headshot ? (
          <img src={headshot} alt="Agent" style={{ width: headshotSz, height: headshotSz, objectFit: "cover", borderRadius: "50%", border: `${Math.round(6 * u)}px solid ${sidebarColor}`, boxShadow: `0 ${Math.round(6 * u)}px ${Math.round(20 * u)}px rgba(0,0,0,0.12)` }} />
        ) : (
          <div style={{ width: headshotSz, height: headshotSz, borderRadius: "50%", backgroundColor: mainLight ? "#e5e7eb" : "rgba(255,255,255,0.08)", border: `${Math.round(6 * u)}px solid ${sidebarColor}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <User style={{ width: headshotSz * 0.35, height: headshotSz * 0.35, color: mainMuted }} />
          </div>
        )}
        <div style={{ width: Math.round(width * 0.08), height: Math.round(3 * u), backgroundColor: sidebarColor, opacity: 0.4, marginTop: Math.round(height * 0.01), marginBottom: Math.round(height * 0.006) }} />
        <p style={{ fontSize: nameSz, fontWeight: 800, color: mainText, margin: 0, letterSpacing: "0.06em", textTransform: "uppercase" as const, whiteSpace: "nowrap" }}>{nameText}</p>
        <p style={{ fontSize: detailSz, color: mainMuted, margin: 0, marginTop: Math.round(height * 0.005), textTransform: "uppercase" as const, letterSpacing: "0.08em", fontWeight: 500 }}>Real Estate Agent</p>
        <div style={{ width: Math.round(width * 0.08), height: Math.round(3 * u), backgroundColor: sidebarColor, opacity: 0.4, marginTop: Math.round(height * 0.006), marginBottom: Math.round(height * 0.008) }} />
        <p style={{ fontSize: phoneSz, fontWeight: 700, color: mainText, margin: 0, letterSpacing: "0.04em" }}>{phoneText}</p>
        {website && <p style={{ fontSize: detailSz, color: mainMuted, margin: 0, marginTop: Math.round(height * 0.008), overflowWrap: "break-word" }}>{website}</p>}
        {email && <p style={{ fontSize: detailSz, color: mainMuted, margin: 0, marginTop: Math.round(height * 0.005), overflowWrap: "break-word" }}>{email}</p>}
        {bulletPoints && bulletPoints.filter(Boolean).length > 0 && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: Math.round(width * 0.015), marginTop: Math.round(height * 0.015), flexWrap: "wrap" }}>
            {bulletPoints.filter(Boolean).map((bp, i) => (
...
export function PropertyPdfPage({ pageNumber, address, cityStateZip, price, beds, baths, sqft, description, features, photos, accentColor, fontFamily }: {
  ...
  const estimatedFeatureH = featureLines * 80 + (featureLines > 0 ? 90 : 0);
      ...
      <div style={{ width: W, height: H, backgroundColor: "#ffffff", fontFamily, position: "relative", padding: margin }}>
        ...
        <div style={{ position: "absolute", top: margin + headerBarH, left: margin, right: margin, height: heroH, overflow: "hidden" }}>
          {heroPhoto ? (
            ...
            <div style={{ width: "100%", height: "100%", backgroundColor: "#f0efea", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <span style={{ color: "#ccc", fontSize: 56, fontWeight: 500, letterSpacing: "0.08em" }}>HERO PHOTO</span>
            </div>
          )}
        ...
        <div style={{ position: "absolute", top: heroH + headerBarH + margin, left: margin, right: margin, bottom: margin + headerBarH, display: "flex" }}>
          <div style={{ width: leftW, padding: `${pad * 0.6}px ${pad * 0.6}px ${pad * 0.5}px ${pad * 0.3}px`, display: "flex", flexDirection: "column", justifyContent: "flex-start", overflow: "hidden" }}>
            <p style={{ fontSize: 50, color: accent, fontWeight: 500, letterSpacing: "0.12em", textTransform: "uppercase" as const, margin: 0, lineHeight: 1 }}>Introducing</p>
            <p style={{ fontSize: responsiveSize(102, addressText, 18), color: "#1a1a1a", fontWeight: 800, lineHeight: 1.05, margin: 0, marginTop: 12, overflowWrap: "break-word" }}>{addressText}</p>
            <p style={{ fontSize: 42, color: "#666", fontWeight: 500, margin: 0, marginTop: 10, letterSpacing: "0.02em" }}>{cityText}</p>
            <div style={{ width: 70, height: 4, backgroundColor: accent, marginTop: 28, borderRadius: 2 }} />
            <p style={{ fontSize: 112, fontWeight: 800, color: accent, margin: 0, marginTop: 14, lineHeight: 1.0, letterSpacing: "-0.02em" }}>{priceText}</p>
            {detailsText && <p style={{ fontSize: 42, color: "#555", fontWeight: 600, margin: 0, marginTop: 10, letterSpacing: "0.06em" }}>{detailsText}</p>}
            {features && (
              <div style={{ marginTop: 36 }}>
                <p style={{ fontSize: 38, fontWeight: 700, color: "#333", letterSpacing: "0.10em", textTransform: "uppercase" as const, margin: 0, marginBottom: 16 }}>Key Features</p>
                <div style={{ fontSize: 38, color: "#444", lineHeight: 2.0 }}>
                  {features.split("\n").filter(Boolean).map((f, i) => (
                    <div key={i} style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
                      <span style={{ color: accent, flexShrink: 0, fontWeight: 700, fontSize: 28, marginTop: 5 }}>—</span>
          ...
          <div style={{ width: rightW, display: "flex", flexDirection: "column", padding: `${Math.round(pad * 0.4)}px ${Math.round(pad * 0.3)}px ${Math.round(pad * 0.4)}px ${Math.round(pad * 0.4)}px`, gap: photoGap }}>
            <div style={{ flex: 1, borderRadius: 16, overflow: "hidden" }}>
              {photo2 ? <img src={photo2} alt="Photo 2" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <div style={{ width: "100%", height: "100%", backgroundColor: "#f0efea", display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ color: "#ccc", fontSize: 44, fontWeight: 500 }}>Photo 2</span></div>}
            </div>
            <div style={{ flex: 1, borderRadius: 16, overflow: "hidden" }}>
              {photo3 ? <img src={photo3} alt="Photo 3" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <div style={{ width: "100%", height: "100%", backgroundColor: "#f5f5f0", display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ color: "#ccc", fontSize: 44, fontWeight: 500 }}>Photo 3</span></div>}
            </div>
          </div>
        </div>
    ...
    <div style={{ width: W, height: H, backgroundColor: "#ffffff", fontFamily, position: "relative" }}>
      ...
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: footerH + headerBarH, display: "flex", alignItems: "center", justifyContent: "center", paddingBottom: headerBarH }}>
        <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
          ...
          <p style={{ fontSize: 36, color: "#999", fontWeight: 500, letterSpacing: "0.06em", margin: 0 }}>{address}{cityStateZip ? ` · ${cityStateZip}` : ""}</p>
          <div style={{ width: 40, height: 2, backgroundColor: "#ddd" }} />
          <p style={{ fontSize: 34, color: "#bbb", fontWeight: 500, margin: 0 }}>Page {pageNumber + 1} of {totalPages}</p>
        </div>
      </div>
  ...
  return (
    <div style={{ width: w, height: h, background: "transparent", position: "relative" }}>
      <div style={{ position: "absolute", top: inset, left: inset, right: inset, bottom: inset, borderRadius: radius, border: `${borderW}px solid ${borderColor}`, backgroundColor: bgColor || "#14532d", overflow: "hidden", fontFamily }}>
        ...
        {!bgPhoto && (
          <>
            <div style={{ position: "absolute", top: -Math.round(h * 0.15), right: -Math.round(w * 0.02), width: Math.round(w * 0.18), height: Math.round(h * 0.55), backgroundColor: accentBg, transform: "rotate(25deg)", opacity: 0.4 }} />
            <div style={{ position: "absolute", top: Math.round(h * 0.35), right: -Math.round(w * 0.05), width: Math.round(w * 0.15), height: Math.round(h * 0.55), backgroundColor: accentBg, transform: "rotate(25deg)", opacity: 0.25 }} />
            <div style={{ position: "absolute", bottom: -Math.round(h * 0.1), right: Math.round(w * 0.08), width: Math.round(w * 0.12), height: Math.round(h * 0.45), backgroundColor: accentBg, transform: "rotate(25deg)", opacity: 0.15 }} />
          </>
        )}
        <div style={{ position: "relative", zIndex: 1, display: "flex", alignItems: "center", width: "100%", height: "100%", padding: `${padY}px ${padX}px` }}>
          <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", minWidth: 0, paddingRight: Math.round(40 * u) }}>
            {(logo || brokerage) && (
          ...
          <div style={{ flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
            {headshot ? (
              ...
              <div style={{ width: headshotSz, height: headshotSz, borderRadius: "50%", backgroundColor: "rgba(255,255,255,0.04)", border: `${headshotBorder}px solid ${borderColor}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <User style={{ width: headshotSz * 0.35, height: headshotSz * 0.35, color: textMuted }} />
              </div>
            )}
          </div>
        </div>
