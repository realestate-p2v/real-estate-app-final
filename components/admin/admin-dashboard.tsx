...
/**
 * Returns a short package label for display in the collapsed row.
 * - URL orders store the label directly as `listing_package_label`
 * - Upload orders derive the tier from photo count
 */
function getPackageLabel(order: any): string {
  // URL-based orders store the label explicitly
  if (order.listing_package_label) return order.listing_package_label

  // Upload-based orders: derive from photo count
