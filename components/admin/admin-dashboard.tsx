...
import { 
  ...
  Download, Hash, User, Building2, Laptop, Search, DollarSign, Mail, Phone, Globe
} from "lucide-react"
...
// THE FIX: This function explicitly pulls the text fields from your JSON data
...
function OrderCard({ order, isActive }: { order: any, isActive: boolean }) {
  ...
  return (
    <Collapsible open={isOpen} onOpenChange={setIsOpen}>
      <Card className={`border-none overflow-hidden transition-all duration-500 
        ...
        <CollapsibleTrigger className="w-full bg-white p-6 flex items-center gap-6 text-left">
          ...
          <div className="flex-1 grid grid-cols-2 md:grid-cols-4 items-center gap-4">
            <div>
              <h3 className="font-black text-lg text-slate-900 leading-none mb-1 uppercase">{order.customer_name || "New Client"}</h3>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">#{order.order_id?.slice(0, 8)}</p>
            </div>
            
            <div className="flex flex-col">
              <span className="text-[9px] font-black text-slate-400 uppercase mb-1">Price Paid</span>
              <span className="text-emerald-600 font-black text-lg">${order.total_price || "0.00"}</span>
            </div>

            <div className="hidden md:block">
              <span className="text-[9px] font-black text-slate-400 uppercase mb-1">Package</span>
              <p className={`text-xs font-black uppercase ${brand.tier === 'Custom Branding' ? 'text-indigo-600' : 'text-slate-500'}`}>
                {brand.tier}
              </p>
            </div>
        ...
        <CollapsibleContent className="bg-white border-t border-slate-100">
          <div className="p-8 grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            {/* BRANDING SECTION - NOW SHOWING TEXT FIELDS */}
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <div className="bg-slate-50 p-5 rounded-2xl border border-slate-200 space-y-4">
                  <BrandingRow label="Agent Name" value={brand.agentName} icon={<User className="w-4 h-4 text-slate-400"/>} />
                  <BrandingRow label="Company" value={brand.company} icon={<Building2 className="w-4 h-4 text-slate-400"/>} />
                  <BrandingRow label="Website" value={brand.website} icon={<Globe className="w-4 h-4 text-slate-400"/>} />
                  
                  <div className="pt-2">
                    <label className="text-[9px] font-black text-slate-400 uppercase block mb-2">Instructions</label>
                    <div className="text-xs font-bold text-slate-700 bg-white p-4 rounded-xl border border-slate-200 min-h-[80px]">
                      {order.branding_instructions || "No custom instructions."}
                    </div>
                  </div>

                  {brand.logo && (
                    <a href={brand.logo} target="_blank" className="flex items-center justify-between p-3 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-all">
                      <span className="text-[10px] font-black uppercase">Download Client Logo</span>
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-2 border-b border-blue-100 pb-3">
                ...
                <h4 className="text-[11px] font-black uppercase tracking-widest text-blue-600">Production</h4>
              </div>
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                   <DataBox label="VOICE" value={order.voiceover} />
                   <DataBox label="MUSIC" value={order.music_selection} />
                </div>
                <div className="bg-blue-50/30 p-4 rounded-xl border border-blue-100">
                  ...
                  <p className="text-xs font-bold text-slate-700 leading-loose">
                    {order.voiceover_script || "Refer to property visual data."}
                  </p>
                </div>
                <div className="grid grid-cols-2 gap-2">
                   <AssetBtn label="Music" url={order.music_file} icon={<Music className="w-3.5 h-3.5" />} />
                   <AssetBtn label="Script" url={order.script_file} icon={<FileText className="w-3.5 h-3.5" />} />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-2 border-b border-emerald-100 pb-3">
                ...
                <h4 className="text-[11px] font-black uppercase tracking-widest text-emerald-600">Delivery</h4>
              </div>
              <div className="bg-slate-900 p-6 rounded-3xl space-y-6">
                <div className="space-y-1">
                   <p className="text-xs font-bold text-slate-300 flex items-center gap-2"><Mail className="w-3 h-3"/> {order.customer_email}</p>
                   <p className="text-xs font-bold text-slate-300 flex items-center gap-2"><Phone className="w-3 h-3"/> {order.customer_phone || "No Phone"}</p>
                </div>
                <div className="space-y-2">
                  ...
                  />
                  <Button onClick={saveLink} className="w-full bg-indigo-600 font-black h-11 rounded-xl">
                    {isSaving ? <Loader2 className="animate-spin h-4 w-4" /> : "SYNC DELIVERY LINK"}
                  </Button>
                </div>
                <div className="grid grid-cols-2 gap-3">
                   <Button asChild variant="outline" className="h-12 border-slate-700 text-white hover:bg-white/10 font-black text-xs">
                     <a href={order.photos_url} target="_blank">RAW ASSETS</a>
                   </Button>
                   <Button onClick={toggleStatus} className={`h-12 font-black text-xs ${isActive ? 'bg-emerald-500' : 'bg-slate-700'}`}>
                     {isActive ? 'FINISH PROJECT' : 'RE-OPEN'}
                   </Button>
                </div>
              </div>
            </div>
...
// Row component for branding details
function BrandingRow({ label, value, icon }: { label: string, value: string, icon: any }) {
  return (
...
function AssetBtn({ label, url, icon }: { label: string, url: string, icon: any }) {
