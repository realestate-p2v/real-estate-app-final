  ...
  try {
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        // Determine product name from photo count
        const photoCountNum = order?.photoCount || parseInt(metaPhotoCount) || 0;
        let productName = "Standard";
        if (photoCountNum > 25) {
          productName = "Professional";
        } else if (photoCountNum > 12) {
          productName = "Premium";
        }
        ...
        const personalizationData = {
          order_id: orderId,
          product: `Real Estate Video - ${productName}`,
