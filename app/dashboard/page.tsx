...
export default function DashboardPage() {
  ...
  // ── Subscriber gets full Mission Control ──
  if (subscription.active && user) {
    const tools = [
      ...
      { icon: PenTool, label: "Design Studio", desc: "Marketing graphics, remix clips, listing flyers", href: "/dashboard/lens/design-studio", color: "text-indigo-400", bg: "bg-indigo-400/10", ring: "ring-indigo-400/20", stat: null },
      ...
      { icon: Sofa, label: "Virtual Staging", desc: "Furnish empty rooms with AI in seconds", href: "/dashboard/lens/staging", color: "text-violet-400", bg: "bg-violet-400/10", ring: "ring-violet-400/20", stat: null },
      { icon: FileText, label: "Reports", desc: "Branded buyer & seller guides", href: "/dashboard/lens/reports", color: "text-amber-400", bg: "bg-amber-400/10", ring: "ring-amber-400/20", stat: null, badge: "PRO" },
      { icon: Film, label: "Quick Videos", desc: "Short clips from $4.95/clip", href: "/order", color: "text-teal-400", bg: "bg-teal-400/10", ring: "ring-teal-400/20", stat: null },
    ];
        ...
        <div className="relative z-10 mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
          ...
          <div className="mc-animate mt-6 grid gap-3 sm:grid-cols-2" style={{ animationDelay: "0.2s" }}>
            <Link href="/order" className="group flex items-center gap-4 rounded-2xl border border-cyan-400/10 bg-cyan-400/[0.04] p-4 sm:p-5 transition-all hover:border-cyan-400/25 hover:bg-cyan-400/[0.08]">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 text-xs text-white/40">Starting at <span className="font-bold text-cyan-400">$4.95/clip</span></p>
              </div>
          ...
          {activeTab === "tools" && (
            <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {tools.map((tool, i) => (
                <Link key={tool.label} href={tool.href} className="mc-chip-animate group relative flex flex-col gap-3 rounded-2xl border border-white/[0.06] bg-white/[0.03] p-5 backdrop-blur-sm transition-all hover:border-cyan-400/20 hover:bg-white/[0.06]" style={{ animationDelay: `${0.28 + i * 0.05}s` }}>
                  {"badge" in tool && tool.badge && <span className="absolute top-3 right-3 text-[9px] font-black uppercase tracking-wider text-amber-400 bg-amber-400/10 px-2 py-0.5 rounded-full border border-amber-400/20">{tool.badge}</span>}
                  <div className="flex items-center gap-3">
                  ...
                  <span className="inline-flex items-center gap-1 text-xs font-semibold text-cyan-400/60 group-hover:text-cyan-400 transition-colors mt-auto pt-1">Open<ArrowRight className="h-3 w-3 transition-transform group-hover:translate-x-0.5" /></span>
                </Link>
              ))}
            </div>
          )}
  ...
  /* ═══════════════════════════════════════════════════════
     NON-SUBSCRIBER VIEW — Light theme upsell
     ═══════════════════════════════════════════════════════ */
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-12">
        <div className="text-center space-y-3 mb-8">
          <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight text-foreground">Your Dashboard</h1>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">Manage your videos, track orders, and grow your business.</p>
        </div>
          ...
          <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-2xl p-5 mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-green-700">Use code <span className="font-mono font-bold bg-green-100 px-2 py-0.5 rounded">{signupPrizeCode}</span> at checkout</p>
              </div>
            </div>
          </div>
        )}

        <div className="mt-6"><LensTrialBanner /></div>
        ...
        <div className="bg-card rounded-2xl border border-border p-6 sm:p-8 mt-6">
          <div className="flex items-start gap-4 flex-wrap">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-muted-foreground mt-1">AI photo coaching, design studio, listing descriptions, virtual staging, custom reports, and priority delivery.</p>
              <div className="flex flex-wrap gap-3 mt-4">
                <Button asChild className="bg-accent hover:bg-accent/90 text-accent-foreground font-black"><Link href="/lens">Subscribe to P2V Lens<ArrowRight className="ml-2 h-4 w-4" /></Link></Button>
                <Button asChild variant="outline"><Link href="/lens#waitlist-form">Join the Waitlist</Link></Button>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Quick Video CTA */}
        <Link href="/lens" className="block mt-4">
          <div className="bg-gradient-to-r from-cyan-50 to-blue-50 border border-cyan-200 rounded-2xl p-6 hover:border-cyan-400 hover:shadow-lg transition-all">
            <div className="flex items-start gap-4">
              <div className="h-12 w-12 rounded-xl bg-cyan-100 flex items-center justify-center flex-shrink-0"><Film className="h-6 w-6 text-cyan-600" /></div>
              <div>
                <div className="flex items-center gap-2"><h3 className="font-bold text-foreground text-lg">Quick Video</h3><span className="text-[10px] bg-cyan-500 text-white font-black px-2 py-0.5 rounded-full">NEW</span></div>
                <p className="text-sm text-muted-foreground mt-1">Order 5–14 clip videos at just $4.95/clip.</p>
                <p className="text-sm font-bold text-cyan-700 mt-2">Subscribe to unlock →</p>
              </div>
            </div>
          </div>
        </Link>
        ...
        <div className="mt-10 mb-14">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              { icon: Video, title: "Your Videos", description: "Watch, download, and share all your completed listing videos.", href: "/dashboard/videos" },
              { icon: FileText, title: "Saved Drafts", description: "Resume unfinished orders. Photos, settings, and preferences are all saved.", href: "/dashboard/drafts" },
              { icon: RefreshCw, title: "Request Revisions", description: "Select clips, choose new directions, and submit revision requests.", href: "/dashboard/videos" },
              { icon: BarChart3, title: "Order History", description: "Track every order with status updates, delivery dates, and receipts.", href: "/dashboard/videos" },
              { icon: Star, title: "Leave Reviews", description: "Share your experience and unlock exclusive discounts up to 25% off.", href: "/dashboard/reviews" },
              { icon: DollarSign, title: "Referral Program", description: "Earn 20% commission when people you refer order listing videos.", href: "/dashboard/referral-earnings" },
              { icon: Sparkles, title: "P2V Lens", description: "AI photo coaching, design studio, description writer, virtual staging, and more.", href: "/dashboard/lens" },
              { icon: Building2, title: "Brokerage Dashboard", description: "View your team's tier progress, monthly usage, and recent orders.", href: "/dashboard/brokerage" },
            ].map(({ icon: Icon, title, description, href }, i) => (
              <Link key={i} href={href} className="relative bg-card rounded-xl border border-primary/20 p-5 space-y-2.5 hover:border-accent/40 hover:shadow-lg transition-all duration-300 group block">
        ...
        <div className="mb-14">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {(hasDirectoryListing ? [
              { icon: Edit, title: "Edit My Listing", description: "Update your profile, specialties, bio, and contact info.", href: "/directory/edit" },
              { icon: BarChart3, title: "Directory Analytics", description: "See how many realtors viewed your listing.", href: "/dashboard/directory-analytics" },
              { icon: DollarSign, title: "Referral Earnings", description: "Track your commissions and payouts.", href: "/dashboard/referral-earnings" },
            ] : [
              { icon: UserPlus, title: "Join the Directory", description: "List yourself for free and get discovered by realtors.", href: "/directory/join" },
              { icon: DollarSign, title: "Referral Program", description: "Earn 20% commission on referred orders.", href: "/partners" },
            ]).map(({ icon: Icon, title, description, href }, i) => (
              <Link key={i} href={href} className="relative bg-card rounded-xl border border-primary/20 p-5 space-y-2.5 hover:border-accent/40 hover:shadow-lg transition-all duration-300 group block">
                <span className="absolute top-3 right-3 text-[10px] font-semibold text-green-600 bg-green-100 px-2 py-0.5 rounded-full">Live</span>
        ...
        <div className="bg-card rounded-2xl border border-border p-8 sm:p-10 text-center space-y-5 mb-12">
          <h2 className="text-2xl sm:text-3xl font-bold text-foreground">Ready to create your next listing video?</h2>
          <p className="text-muted-foreground max-w-lg mx-auto">Upload your photos, pick your music and branding, and we'll deliver a professional walkthrough video within 24 hours.</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Button asChild className="bg-accent hover:bg-accent/90 px-8 py-6 text-lg font-bold"><Link href="/order"><Play className="mr-2 h-5 w-5" />Create My Listing Video</Link></Button>
            <Button asChild variant="outline" className="px-8 py-6 text-lg"><Link href="/portfolio">View Portfolio</Link></Button>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <Link href="/resources/photography-guide" className="group bg-card rounded-xl border border-border p-5 flex items-start gap-4 hover:border-primary/30 hover:shadow-sm transition-all">
              ...
              <div><h4 className="font-semibold text-foreground group-hover:text-primary transition-colors">Free Photography Guide</h4><p className="text-sm text-muted-foreground mt-1">32-page guide with camera settings, lighting, staging, and drone tips.</p></div>
            </Link>
            <Link href="/support" className="group bg-card rounded-xl border border-border p-5 flex items-start gap-4 hover:border-primary/30 hover:shadow-sm transition-all">
              <div className="h-10 w-10 rounded-lg bg-primary/10 flex items-center justify-center flex-shrink-0"><Sparkles className="h-5 w-5 text-primary" /></div>
              <div><h4 className="font-semibold text-foreground group-hover:text-primary transition-colors">Support & FAQ</h4><p className="text-sm text-muted-foreground mt-1">Find answers, contact us, or chat on WhatsApp.</p></div>
            </Link>
          </div>
        </div>
      </div>
