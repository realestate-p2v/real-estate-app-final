  ...
  const loadUserVideos=async()=>{
    ...
    try{
      ...
      const sources=vids.map((o:any)=>{
        ...
        if(clipUrls.length>0){return{orderId:o.order_id,address:addr,date:dt,clips:clipUrls.map((url:string,i:number)=>({url,thumbnail:orderThumb,label:`Clip ${i+1}`}))};}
  ...
  const renderPreview=()=>{
    if(activeTab==="video-remix"){
      ...
      return<div style={{width:currentRemixSize.width,height:currentRemixSize.height,backgroundColor:"#0c0c10",position:"relative",overflow:"hidden"}}>
        {firstClip.thumbnail?<img src={firstClip.thumbnail} alt="" style={{width:"100%",height:"100%",objectFit:"cover",opacity:0.8}}/>:<video src={firstClip.sourceUrl} muted autoPlay loop playsInline crossOrigin="anonymous" style={{width:"100%",height:"100%",objectFit:"cover",opacity:0.8}}/>}
                ...
                {remixClips.map((clip,idx)=>{const isExp=expandedClipId===clip.id;const effDur=(clip.trimEnd-clip.trimStart)/clip.speed;
                  return<div key={clip.id} style={{borderRadius:10,border:"1px solid var(--sbr)",background:"rgba(255,255,255,0.02)",overflow:"hidden"}}>
                    <button onClick={()=>setExpandedClipId(isExp?null:clip.id)} style={{width:"100%",display:"flex",alignItems:"center",gap:8,padding:"10px 12px",background:"none",border:"none",cursor:"pointer",fontFamily:"var(--sf)",textAlign:"left" as const}}>
                      ...
                      <div style={{width:36,height:24,borderRadius:4,overflow:"hidden",flexShrink:0,backgroundColor:"rgba(255,255,255,0.06)",display:"flex",alignItems:"center",justifyContent:"center"}}>{clip.thumbnail?<img src={clip.thumbnail} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:<video src={clip.sourceUrl} muted preload="metadata" crossOrigin="anonymous" style={{width:"100%",height:"100%",objectFit:"cover"}} onLoadedData={e=>{const v=e.target as HTMLVideoElement;v.currentTime=1;}}/>}</div>
