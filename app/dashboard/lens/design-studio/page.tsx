...
function DesignStudioContent(){
  ...
  const handleSelectProperty=(id:string)=>{
    ...
    // Reset all property-derived fields FIRST so stale data from previous property is cleared
    setBeds("");setBaths("");setSqft("");setPrice("");
    setFlyerBeds("");setFlyerBaths("");setFlyerSqft("");setFlyerPrice("");
    setPdfBeds("");setPdfBaths("");setPdfSqft("");setPdfPrice("");
    setBrandPrice("");
    setFlyerAmenities([]);setFlyerPhotos([]);setPdfPhotos([]);
    setFlyerDescription("");setPdfDescription("");
    setFlyerListingUrl("");setFlyerVideoUrl("");setFlyerStagingUrl("");
    setPdfFeatures("");setBrandFeatures("");

    // Now set values from new property
    ...
    (async()=>{
      try{
        ...
        if(photos.length<5){
          // Build a normalized search key from the street-number + first street word
          // e.g. "203 Dionnes Way" -> "203 Dionnes"; forgiving of casing, trailing text, unit numbers
          const addrParts=addr.trim().split(/\s+/);
          const streetKey=addrParts.slice(0,2).join(" ");
          const normAddr=addr.toLowerCase().replace(/[^a-z0-9]/g,"");
          // Fetch all user's orders, filter client-side so we can match flexibly
          const{data:orders}=await supabase.from("orders").select("photos,property_address").eq("user_id",u.id).order("created_at",{ascending:false});
          const matching=(orders||[]).filter((o:any)=>{
            const oa=(o.property_address||"").toString();
            if(!oa)return false;
            // Strict substring match (first try)
            if(oa.toLowerCase().includes(streetKey.toLowerCase()))return true;
            // Fallback: normalized alphanumeric-only comparison (handles punctuation/spacing diffs)
            const normOa=oa.toLowerCase().replace(/[^a-z0-9]/g,"");
            return normOa.includes(normAddr.substring(0,Math.min(12,normAddr.length)))||normAddr.includes(normOa.substring(0,Math.min(12,normOa.length)));
          });
          for(const o of matching){const urls=(o.photos||[]).map((p:any)=>p.secure_url||p.url).filter(Boolean);photos=[...photos,...urls];if(photos.length>=7)break;}
  ...
  const css=`
    ...
    *{margin:0;padding:0;box-sizing:border-box;}.sr{font-family:var(--sf);background:var(--sb);color:var(--st);height:100vh;display:flex;flex-direction:column;overflow:hidden;-webkit-font-smoothing:antialiased;}
    ...
    .tool-header-row{padding:20px 24px;background:var(--ss);border-bottom:1px solid var(--sbr);flex-shrink:0;}
    ...
    @media(max-width:849px){
      ...
      .tool-header-row{padding:14px 14px;}
    }
  `;
    ...
    return(
      <div style={{display:"flex",height:"100vh",alignItems:"center",justifyContent:"center",background:"#0f0f1a",color:"#fff"}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          ...
          <span style={{fontSize:18,fontWeight:600}}>Loading Design Studio...</span>
        </div>
      </div>
    );
  }
    ...
    <div className="sr">
      ...
      <div className="sb">
        ...
        <div className={`slp${isMobile&&mobilePanel?" mob-open":""}`}>
          ...
          {activeTab==="listing-flyer"&&leftPanel==="uploads"&&<><div className="ph"><ImageIcon size={15} color="var(--sa)"/>Photos ({flyerPhotos.length}/7)</div><div style={{padding:14}}>{!selectedPropertyId?<div style={{padding:"20px 12px",textAlign:"center" as const,borderRadius:10,border:"1px dashed var(--sbr)",background:"rgba(255,255,255,0.02)"}}><ImageIcon size={22} color="var(--std)" style={{margin:"0 auto 8px",opacity:0.6}}/><p style={{fontSize:11,color:"var(--std)",margin:0,lineHeight:1.5}}>Select a property above to load its photos automatically.</p></div>:flyerPhotos.length===0?<div style={{padding:"20px 12px",textAlign:"center" as const,borderRadius:10,border:"1px dashed var(--sbr)",background:"rgba(255,255,255,0.02)"}}><ImageIcon size={22} color="var(--std)" style={{margin:"0 auto 8px",opacity:0.6}}/><p style={{fontSize:11,color:"var(--std)",margin:0,lineHeight:1.5}}>No photos found for this property yet.</p></div>:<><p style={{fontSize:11,color:"var(--std)",marginBottom:10,lineHeight:1.5}}>Photos auto-loaded from this property’s order. First = hero, 2–3 stack right, 4–7 fill bottom row.</p><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6}}>{flyerPhotos.map((url:string,i:number)=>(<div key={i} style={{position:"relative",aspectRatio:"1",borderRadius:8,overflow:"hidden",border:"1px solid var(--sbr)"}}><img src={url} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/><div style={{position:"absolute",top:2,left:2,background:"rgba(0,0,0,0.7)",color:"#fff",fontSize:9,fontWeight:700,padding:"1px 5px",borderRadius:4}}>{i===0?"Hero":i<3?`R${i}`:`B${i-2}`}</div></div>))}</div></>}<div style={{marginTop:16}}><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><UploadZone label="Headshot" imageUrl={headshot} onUpload={f=>gatedAction(()=>setHeadshot(URL.createObjectURL(f)))} onClear={()=>setHeadshot(null)} uploading={false} compact/><UploadZone label="Logo" imageUrl={logo} onUpload={f=>gatedAction(()=>setLogo(URL.createObjectURL(f)))} onClear={()=>setLogo(null)} uploading={false} compact/></div></div></div></>}
