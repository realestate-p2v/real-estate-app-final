...
export default function PropertiesPage() {
  ...
  async function loadProperties() {
    ...
    try {
      ...
      try {
        ...
        if (orders && orders.length > 0) {
          ...
          for (const order of orders) {
            ...
            // Insert — ignore errors (e.g. unique constraint violations)
            await supabase
              .from("agent_properties")
              .insert({
                user_id: user.id,
      ...
      // --- Count assets for each property ---
      ...
      for (const p of props) {
        ...
        try {
          // Video count — fetch orders and match by normalized address
          ...
          videoCount = (userOrders || []).filter((o: any) => {
