...
function InfoBarTemplate({ size, listingPhoto, videoElement, headshot, logo, address, beds, baths, sqft, price, agentName, phone, brokerage, badgeText, badgeColor, fontFamily, barColor, accentColor }: any) {
  ...
  const Photo = () => (
    <div style={{position:"absolute",top:0,left:0,right:0,height:`${pp}%`}}>
      {videoElement ? <div style={{width:"100%",height:"100%",position:"relative",overflow:"hidden"}}>{videoElement}</div> : listingPhoto ? <img src={listingPhoto} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}} /> : <div style={{width:"100%",height:"100%",backgroundColor:"#1a1a2e",display:"flex",alignItems:"center",justifyContent:"center"}}><ImageIcon style={{width:64*unit,height:64*unit,color:"rgba(255,255,255,0.12)"}} /></div>}
...
function OpenHouseTemplate({ size, listingPhoto, videoElement, headshot, logo, address, beds, baths, sqft, price, date, time, agentName, phone, brokerage, fontFamily, barColor, accentColor }: any) {
  ...
  return (
    <div style={{position:"relative",overflow:"hidden",width:w,height:h,fontFamily}}>
      {videoElement?<div style={{position:"absolute",inset:0,overflow:"hidden"}}>{videoElement}</div>:listingPhoto?<img src={listingPhoto} alt="" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover"}} />:<div style={{position:"absolute",inset:0,backgroundColor:"#1a1a2e",display:"flex",alignItems:"center",justifyContent:"center"}}><ImageIcon style={{width:64*unit,height:64*unit,color:"rgba(255,255,255,0.12)"}} /></div>}
      ...
      <div style={{position:"absolute",inset:0,backgroundImage:"radial-gradient(ellipse at center,transparent 50%,rgba(0,0,0,0.20) 100%)"}} />
...
function YardSignSplitBar({width,height,headshot,logo,agentName,phone,email,brokerage,officeName,officePhone,headerText,topColor,bottomColor,fontFamily,qrDataUrl,bulletPoints}:any) {
  ...
  return (
    <div style={{width,height,fontFamily,display:"flex",flexDirection:"column" as const}}>
      ...
      <div style={{height:centerH,backgroundColor:"#fff",display:"flex",flexDirection:"column" as const,alignItems:"center",justifyContent:"center",padding:`${Math.round(centerH*0.06)}px ${Math.round(width*0.08)}px`,gap:Math.round(centerH*0.03),position:"relative"}}>
      ...
      <div style={{height:bottomH,backgroundColor:bottomColor,display:"flex",alignItems:"center",justifyContent:"center",padding:`0 ${Math.round(width*0.06)}px`}}>
        <div style={{textAlign:"center" as const}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:Math.round(width*0.025)}}>
            <div style={{width:Math.round(width*0.05),height:Math.round(2*u),backgroundColor:bR}} />
            <div>{showBothLines?<><p style={{fontSize:bNSz,fontWeight:800,color:bC,margin:0,letterSpacing:"0.05em"}}>{brokerage}</p><p style={{fontSize:Math.round(bNSz*0.7),fontWeight:600,color:bC,margin:0,marginTop:Math.round(height*0.004),letterSpacing:"0.03em",opacity:0.8}}>{officeName}</p></>:<p style={{fontSize:bNSz,fontWeight:800,color:bC,margin:0,letterSpacing:"0.05em"}}>{ot}</p>}</div>
            <div style={{width:Math.round(width*0.05),height:Math.round(2*u),backgroundColor:bR}} />
          </div>
          {officePhone&&<p style={{fontSize:bPSz,fontWeight:600,color:bC,margin:0,marginTop:Math.round(height*0.008)}}>{officePhone}</p>}
        </div>
      </div>
    </div>
  );
...
function PropertyPdfPage({pageNumber,address,cityStateZip,price,beds,baths,sqft,description,features,photos,accentColor,fontFamily}:any) {
  ...
  return (
    <div style={{width:W,height:H,backgroundColor:"#fff",fontFamily,position:"relative"}}>
      ...
      {showOverflow&&<div style={{padding:`${pgPad+hBar}px ${pgPad}px 0`,height:overflowH}}><div style={{fontSize:38,color:"#555",lineHeight:1.7,overflowWrap:"break-word" as const}}>{ovD.map((p:string,i:number)=><p key={i} style={{margin:0,marginBottom:10}}>{p}</p>)}</div></div>}
      <div style={{padding:`${showOverflow?pgGap:pgPad+hBar}px ${pgPad}px ${pgPad}px`,display:"flex",gap:pgGap}}>
        <div style={{width:colW,display:"flex",flexDirection:"column" as const,gap:pgGap}}>{Array.from({length:rows},(_,r)=>r*2).map(idx=>{const p=pgPhotos[idx];return <div key={idx} style={{height:photoH,borderRadius:16,overflow:"hidden",backgroundColor:p?undefined:"#f5f5f0"}}>{p&&<img src={p} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}} />}</div>;})}</div>
...
function BrandingCardTemplate({orientation,logo,headshot,agentName,phone:ph2,email,brokerage,tagline,website,address:addr,cityState,price:pr2,features,bgColor,accentColor,bgPhoto,fontFamily}:any) {
  ...
  if(isV){
    ...
    const Corner = ({top,left,right,bottom}:{top?:number;left?:number;right?:number;bottom?:number}) => (<>
      <div style={{position:"absolute",top,left,right,bottom,width:crnL,height:crnW,backgroundColor:accent,opacity:0.35}} />
      <div style={{position:"absolute",top,left,right,bottom,width:crnW,height:crnL,backgroundColor:accent,opacity:0.35}} />
    </>);
    return (
      <div style={{width:w,height:h,background:"transparent",position:"relative"}}>
        <div style={{position:"absolute",top:inset,left:inset,right:inset,bottom:inset,borderRadius:rad,border:`${bd}px solid ${bC}`,backgroundColor:bgColor||"#14532d",overflow:"hidden",display:"flex",flexDirection:"column" as const,alignItems:"center",justifyContent:"center",padding:pad,fontFamily}}>
          ...
          <Corner top={crnI} left={crnI} /><Corner top={crnI} right={crnI} /><Corner bottom={crnI} left={crnI} /><Corner bottom={crnI} right={crnI} />
