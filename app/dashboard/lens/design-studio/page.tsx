  ...
  const renderPreview=()=>{
    if(activeTab==="video-remix"){
      ...
      return<div style={{width:currentRemixSize.width,height:currentRemixSize.height,backgroundColor:"#0c0c10",position:"relative",overflow:"hidden"}}>
        <video ref={remixVideoRef} src={firstClip.sourceUrl} muted loop playsInline style={{width:"100%",height:"100%",objectFit:"cover",opacity:0.8}}/>
        <button onClick={()=>{const v=remixVideoRef.current;if(!v)return;if(v.paused){v.play();setRemixPlaying(true);}else{v.pause();setRemixPlaying(false);}}} style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:64,height:64,borderRadius:"50%",background:"rgba(0,0,0,0.6)",border:"2px solid rgba(255,255,255,0.3)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.2s",backdropFilter:"blur(4px)"}}><Play size={28} color="#fff" style={{marginLeft:remixPlaying?0:3,display:remixPlaying?"none":"block"}}/>{remixPlaying&&<span style={{width:20,height:20,display:"flex",alignItems:"center",justifyContent:"center",gap:3}}><span style={{width:4,height:16,backgroundColor:"#fff",borderRadius:1}}/><span style={{width:4,height:16,backgroundColor:"#fff",borderRadius:1}}/></span>}</button>
                ...
                {remixClips.map((clip,idx)=>{const isExp=expandedClipId===clip.id;const effDur=(clip.trimEnd-clip.trimStart)/clip.speed;
                  return<div key={clip.id} style={{borderRadius:10,border:"1px solid var(--sbr)",background:"rgba(255,255,255,0.02)",overflow:"hidden"}}>
                    <button onClick={()=>setExpandedClipId(isExp?null:clip.id)} style={{width:"100%",display:"flex",alignItems:"center",gap:8,padding:"10px 12px",background:"none",border:"none",cursor:"pointer",fontFamily:"var(--sf)",textAlign:"left" as const}}>
                      ...
                      <div style={{width:36,height:24,borderRadius:4,overflow:"hidden",flexShrink:0,backgroundColor:"rgba(255,255,255,0.06)",display:"flex",alignItems:"center",justifyContent:"center"}}>{clip.thumbnail?<img src={clip.thumbnail} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:<video src={clip.sourceUrl+"#t=1"} muted preload="metadata" playsInline style={{width:"100%",height:"100%",objectFit:"cover"}}/>}</div>
