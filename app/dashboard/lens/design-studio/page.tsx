...
const TABS=[{id:"templates",label:"Listing Graphics",icon:PenTool},{id:"listing-flyer",label:"Listing Flyer",icon:Printer},{id:"branding-card",label:"Branding",icon:CreditCard},{id:"yard-sign",label:"Yard Sign",icon:MapPin},{id:"property-pdf",label:"Property Sheet",icon:FileText},{id:"drone-mark",label:"Drone Mark",icon:Crosshair}];
const LEFT_PANELS:Record<string,{id:string;label:string;icon:any}[]>={"video-remix":[{id:"clips",label:"Clips",icon:Film},{id:"timeline",label:"Timeline",icon:LayoutTemplate},{id:"music",label:"Music",icon:Music},{id:"styles",label:"Styles",icon:Palette}],templates:[{id:"templates",label:"Templates",icon:LayoutTemplate},{id:"uploads",label:"Uploads",icon:Upload},{id:"text",label:"Details",icon:Type},{id:"styles",label:"Styles",icon:Palette}],"listing-flyer":[{id:"uploads",label:"Photos",icon:ImageIcon},{id:"text",label:"Details",icon:Type},{id:"urls",label:"URLs",icon:Globe},{id:"styles",label:"Styles",icon:Palette}],"yard-sign":[{id:"design",label:"Design",icon:LayoutTemplate},{id:"uploads",label:"Uploads",icon:Upload},{id:"text",label:"Details",icon:Type},{id:"styles",label:"Colors",icon:Palette}],"branding-card":[{id:"uploads",label:"Uploads",icon:Upload},{id:"text",label:"Details",icon:Type},{id:"styles",label:"Styles",icon:Palette}],"property-pdf":[{id:"text",label:"Details",icon:Type},{id:"photos",label:"Photos",icon:ImageIcon},{id:"styles",label:"Styles",icon:Palette}],"drone-mark":[{id:"tools",label:"Tools",icon:Crosshair},{id:"style",label:"Style",icon:Palette},{id:"layers",label:"Layers",icon:Layers}]};
...
export default function DesignStudioV2(){
  ...
  const mediaMode="image" as const;
    ...
    if(droneTool==="polygon"){
      if(!droneIsDrawing){setDroneIsDrawing(true);setDroneDrawingPts([pos]);}
        ...
        if(droneDrawingPts.length>=3&&dist<30*(dronePhotoNatural.w/1000)){
          droneAddShape({id:droneUid(),type:"polygon",points:[...droneDrawingPts],color:droneLineColor,style:droneLineStyle,width:droneLineWidth,fillOpacity:droneFillOpacity});
          setDroneIsDrawing(false);setDroneDrawingPts([]);
        }else{setDroneDrawingPts([...droneDrawingPts,pos]);}
      }
    }else if(droneTool==="pin"){
      droneAddShape({id:droneUid(),type:"pin",x:pos.x,y:pos.y,color:dronePinColor,size:dronePinSize,logo:droneLogo,text:dronePinText,labelColor:droneLabelColor});
    }else if(droneTool==="label"){
      if(!droneLabelText.trim()){notify("Enter label text first");return;}
      droneAddShape({id:droneUid(),type:"label",x:pos.x,y:pos.y,text:droneLabelText,fontSize:droneLabelFontSize,font:droneLabelFont,color:droneLabelColor,bg:droneLabelBg});
  ...
  const droneMouseUp=(e:React.MouseEvent)=>{
    ...
    if(droneTool==="rect"&&droneRectStart){
      ...
      if(Math.abs(x2-x1)>10&&Math.abs(y2-y1)>10)droneAddShape({id:droneUid(),type:"rect-shape",points:[{x:x1,y:y1},{x:x2,y:y1},{x:x2,y:y2},{x:x1,y:y2}],color:droneLineColor,style:droneLineStyle,width:droneLineWidth,fillOpacity:droneFillOpacity});
      ...
      if(dist>10)droneAddShape({id:droneUid(),type:droneTool==="arrow"?"arrow":"line-shape",x1:droneLineStart.x,y1:droneLineStart.y,x2:pos.x,y2:pos.y,color:droneLineColor,style:droneLineStyle,width:droneLineWidth});
  ...
  const renderDronePreview=()=>{
    ...
    if(droneTool==="polygon"&&droneIsDrawing&&droneDrawingPts.length>0){
      parts.push(<g key="pp"><polyline points={droneDrawingPts.map(p=>`${p.x},${p.y}`).join(" ")} fill="none" stroke={droneLineColor} strokeWidth={droneLineWidth} strokeDasharray={droneDash(droneLineStyle,droneLineWidth)} strokeLinejoin="round" strokeLinecap="round"/>{droneDrawingPts.map((p,i)=><circle key={i} cx={p.x} cy={p.y} r={6} fill={i===0?"#fff":droneLineColor} stroke={i===0?droneLineColor:"#fff"} strokeWidth={2}/>)}{droneLineCur&&<line x1={droneDrawingPts[droneDrawingPts.length-1].x} y1={droneDrawingPts[droneDrawingPts.length-1].y} x2={droneLineCur.x} y2={droneLineCur.y} stroke={droneLineColor} strokeWidth={droneLineWidth} strokeDasharray="8 6" opacity={0.5}/>}</g>);
    }
    if(droneTool==="rect"&&droneRectStart&&droneRectCur){
      ...
      parts.push(<rect key="rp" x={x} y={y} width={Math.abs(droneRectCur.x-droneRectStart.x)} height={Math.abs(droneRectCur.y-droneRectStart.y)} fill={hexToRgba(droneLineColor,droneFillOpacity)} stroke={droneLineColor} strokeWidth={droneLineWidth} strokeDasharray={droneDash(droneLineStyle,droneLineWidth)}/>);
    }
    if((droneTool==="line"||droneTool==="arrow"||droneTool==="measure")&&droneLineStart&&droneLineCur){
      parts.push(<line key="lp" x1={droneLineStart.x} y1={droneLineStart.y} x2={droneLineCur.x} y2={droneLineCur.y} stroke={droneLineColor} strokeWidth={droneLineWidth} strokeDasharray="8 6" opacity={0.6}/>);
    }
    return parts;
  };
  ...
  const renderPreview=()=>{
    ...
    if(activeTab==="templates"){const photo=listingPhoto;if(selectedTemplate==="open-house")return<OpenHouseTemplate size={currentSize} listingPhoto={photo} headshot={headshot} logo={logo} address={address} addressLine2={addressLine2} beds={beds} baths={baths} sqft={sqft} price={price} date={date} time={time} agentName={agentName} phone={phone} brokerage={brokerage} fontFamily={fontFamily} barColor={barColor} accentColor={accentColor}/>;return<InfoBarTemplate size={currentSize} listingPhoto={photo} headshot={headshot} logo={logo} address={address} addressLine2={addressLine2} beds={beds} baths={baths} sqft={sqft} price={price} agentName={agentName} phone={phone} brokerage={brokerage} badgeText={badge.text} badgeColor={badge.color} fontFamily={fontFamily} barColor={barColor} accentColor={accentColor}/>;}
    ...
    <div className="sr">
      <div className="st">
        ...
        <div className="sac">
          ...
          <div className="td"/>
          <button className="bx" onClick={handleExport} disabled={exporting} style={isDroneMode?{background:"linear-gradient(135deg,#f59e0b,#ef4444)"}:activeTab==="listing-flyer"?{background:"linear-gradient(135deg,#1e3a5f,#2563eb)"}:undefined}>
            {exporting?<><Loader2 size={14} className="animate-spin"/>Exporting...</>:isDroneMode?<><Download size={14}/>Export PNG</>:activeTab==="listing-flyer"?<><Printer size={14}/>Export Flyer</>:<><Download size={14}/>Export</>}
          </button>
        </div>
      </div>
      ...
      <div className="sb">
        ...
        <div className="slp">
          ...
          {isDroneMode&&leftPanel==="style"&&<>
            <div className="ph"><Palette size={15} color="var(--sa)"/>{droneTool==="select"&&droneSelected?"Edit Shape":"Style Defaults"}</div>
            <div style={{padding:14}}>
              {/* Selected shape editing */}
              {droneTool==="select"&&droneSelected?<>
                {(droneSelected.type==="polygon"||droneSelected.type==="rect-shape"||droneSelected.type==="line-shape"||droneSelected.type==="measure"||droneSelected.type==="arrow")&&<div style={{fontSize:10,color:"var(--sa)",background:"var(--sag)",padding:"6px 12px",borderRadius:6,marginBottom:8,lineHeight:1.5}}>\u2195 Drag handles to reshape. Drag body to move.</div>}
                <div className="fg"><label className="fl">Color</label><div style={{display:"flex",flexWrap:"wrap" as const,gap:5}}>{DRONE_COLORS.map(c=><div key={c.hex} className={`drone-swatch ${droneSelected.color===c.hex?"active":""}`} style={{backgroundColor:c.hex}} onClick={()=>droneUpdateShape(droneSelected.id,{color:c.hex})}/>)}<input type="color" value={droneSelected.color} onChange={e=>droneUpdateShape(droneSelected.id,{color:e.target.value})} style={{width:26,height:26,borderRadius:6,border:"1px solid rgba(255,255,255,0.1)",cursor:"pointer",padding:0,background:"none"}}/></div></div>
                {droneSelected.width!==undefined&&<div className="fg"><label className="fl">Width: {droneSelected.width}px</label><div style={{display:"flex",alignItems:"center",gap:8}}><input type="range" min={1} max={12} value={droneSelected.width} onChange={e=>droneUpdateShape(droneSelected.id,{width:Number(e.target.value)})} style={{flex:1,accentColor:"var(--sa)"}}/><span style={{fontSize:11,color:"var(--std)",fontWeight:700,minWidth:20}}>{droneSelected.width}</span></div></div>}
                {droneSelected.style!==undefined&&<div className="fg"><label className="fl">Line Style</label><div style={{display:"flex",gap:3}}>{DRONE_LINE_STYLES.map(s=><button key={s.id} className={`drone-pill ${droneSelected.style===s.id?"active":""}`} onClick={()=>droneUpdateShape(droneSelected.id,{style:s.id})}>{s.label}</button>)}</div></div>}
                {droneSelected.fillOpacity!==undefined&&<div className="fg"><label className="fl">Fill: {Math.round(droneSelected.fillOpacity*100)}%</label><div style={{display:"flex",alignItems:"center",gap:8}}><input type="range" min={0} max={60} value={Math.round(droneSelected.fillOpacity*100)} onChange={e=>droneUpdateShape(droneSelected.id,{fillOpacity:Number(e.target.value)/100})} style={{flex:1,accentColor:"var(--sa)"}}/><span style={{fontSize:11,color:"var(--std)",fontWeight:700,minWidth:28}}>{Math.round(droneSelected.fillOpacity*100)}%</span></div></div>}
                {droneSelected.type==="measure"&&<><div className="fg"><label className="fl">Text</label><input className="fi" value={droneSelected.measureText||""} onChange={e=>droneUpdateShape(droneSelected.id,{measureText:e.target.value})} placeholder="e.g. 15 m"/></div><div className="fg"><label className="fl">Unit</label><div style={{display:"flex",gap:3}}>{["m","ft","yd","m\u00b2"].map(u=><button key={u} className={`drone-pill ${(droneSelected.unit||"m")===u?"active":""}`} onClick={()=>droneUpdateShape(droneSelected.id,{unit:u})}>{u}</button>)}</div></div></>}
                {droneSelected.type==="pin"&&<><div className="fg"><label className="fl">Size: {droneSelected.size||200}px</label><div style={{display:"flex",alignItems:"center",gap:8}}><input type="range" min={80} max={400} value={droneSelected.size||200} onChange={e=>droneUpdateShape(droneSelected.id,{size:Number(e.target.value)})} style={{flex:1,accentColor:"var(--sa)"}}/><span style={{fontSize:11,color:"var(--std)",fontWeight:700,minWidth:28}}>{droneSelected.size||200}</span></div></div><div className="fg"><label className="fl">Pin Label</label><input className="fi" value={droneSelected.text||""} onChange={e=>droneUpdateShape(droneSelected.id,{text:e.target.value})} placeholder="e.g. 1500 m\u00b2 lot"/></div><div className="fg"><label className="fl">Label Color</label><div style={{display:"flex",gap:5}}>{[{hex:"#ffffff"},{hex:"#facc15"},{hex:"#000000"}].map(c=><div key={c.hex} className={`drone-swatch ${(droneSelected.labelColor||"#ffffff")===c.hex?"active":""}`} style={{backgroundColor:c.hex}} onClick={()=>droneUpdateShape(droneSelected.id,{labelColor:c.hex})}/>)}</div></div></>}
                {droneSelected.type==="label"&&<><div className="fg"><label className="fl">Text</label><input className="fi" value={droneSelected.text||""} onChange={e=>droneUpdateShape(droneSelected.id,{text:e.target.value})}/></div><div className="fg"><label className="fl">Font Size: {droneSelected.fontSize||28}px</label><div style={{display:"flex",alignItems:"center",gap:8}}><input type="range" min={14} max={80} value={droneSelected.fontSize||28} onChange={e=>droneUpdateShape(droneSelected.id,{fontSize:Number(e.target.value)})} style={{flex:1,accentColor:"var(--sa)"}}/><span style={{fontSize:11,color:"var(--std)",fontWeight:700,minWidth:28}}>{droneSelected.fontSize||28}</span></div></div><div className="fg"><label className="fl">Font</label><div style={{display:"flex",gap:3}}>{DRONE_FONT_OPTS.map(f=><button key={f.id} className={`drone-pill ${(droneSelected.font||"sans")===f.id?"active":""}`} onClick={()=>droneUpdateShape(droneSelected.id,{font:f.id})} style={{fontFamily:f.family}}>{f.label}</button>)}</div></div><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"4px 0"}}><span style={{fontSize:12,fontWeight:700}}>Background</span><button onClick={()=>droneUpdateShape(droneSelected.id,{bg:!droneSelected.bg})} style={{width:40,height:22,borderRadius:11,border:"none",cursor:"pointer",background:droneSelected.bg?"var(--sa)":"rgba(255,255,255,0.12)",position:"relative"}}><div style={{width:16,height:16,borderRadius:"50%",background:"#fff",position:"absolute",top:3,left:droneSelected.bg?21:3,transition:"left 0.2s",boxShadow:"0 1px 3px rgba(0,0,0,0.3)"}}/></button></div></>}
                <div style={{display:"flex",gap:6,marginTop:10}}><button className="sp" style={{flex:1}} onClick={()=>droneDuplicateShape(droneSelected.id)}>\u29c9 Duplicate</button><button className="sp" style={{flex:1,color:"#ef4444",borderColor:"rgba(239,68,68,0.3)"}} onClick={()=>droneDeleteShape(droneSelected.id)}>\ud83d\uddd1 Delete</button></div>
              </>:<>
                {/* Default style controls for new shapes */}
                {(droneTool==="polygon"||droneTool==="rect"||droneTool==="line"||droneTool==="measure"||droneTool==="arrow")&&<Section title="Line Style" icon={Paintbrush}>
                  <div className="fg"><label className="fl">Color</label><div style={{display:"flex",flexWrap:"wrap" as const,gap:5}}>{DRONE_COLORS.map(c=><div key={c.hex} className={`drone-swatch ${droneLineColor===c.hex?"active":""}`} style={{backgroundColor:c.hex}} onClick={()=>setDroneLineColor(c.hex)}/>)}<input type="color" value={droneLineColor} onChange={e=>setDroneLineColor(e.target.value)} style={{width:26,height:26,borderRadius:6,border:"1px solid rgba(255,255,255,0.1)",cursor:"pointer",padding:0,background:"none"}}/></div></div>
                  <div className="fg"><label className="fl">Style</label><div style={{display:"flex",gap:3}}>{DRONE_LINE_STYLES.map(s=><button key={s.id} className={`drone-pill ${droneLineStyle===s.id?"active":""}`} onClick={()=>setDroneLineStyle(s.id)}>{s.label}</button>)}</div></div>
                  <div className="fg"><label className="fl">Width: {droneLineWidth}px</label><div style={{display:"flex",alignItems:"center",gap:8}}><input type="range" min={1} max={12} value={droneLineWidth} onChange={e=>setDroneLineWidth(Number(e.target.value))} style={{flex:1,accentColor:"var(--sa)"}}/><span style={{fontSize:11,color:"var(--std)",fontWeight:700,minWidth:20}}>{droneLineWidth}</span></div></div>
                  {(droneTool==="polygon"||droneTool==="rect")&&<div className="fg"><label className="fl">Fill: {Math.round(droneFillOpacity*100)}%</label><div style={{display:"flex",alignItems:"center",gap:8}}><input type="range" min={0} max={60} value={Math.round(droneFillOpacity*100)} onChange={e=>setDroneFillOpacity(Number(e.target.value)/100)} style={{flex:1,accentColor:"var(--sa)"}}/><span style={{fontSize:11,color:"var(--std)",fontWeight:700,minWidth:28}}>{Math.round(droneFillOpacity*100)}%</span></div></div>}
                  {droneTool==="measure"&&<div className="fg"><label className="fl">Unit</label><div style={{display:"flex",gap:3}}>{["m","ft","yd","m\u00b2"].map(u=><button key={u} className={`drone-pill ${droneMeasureUnit===u?"active":""}`} onClick={()=>setDroneMeasureUnit(u)}>{u}</button>)}</div></div>}
                </Section>}
                {droneTool==="pin"&&<Section title="Pin Style" icon={MapPin}>
                  <div className="fg"><label className="fl">Color</label><div style={{display:"flex",flexWrap:"wrap" as const,gap:5}}>{DRONE_COLORS.map(c=><div key={c.hex} className={`drone-swatch ${dronePinColor===c.hex?"active":""}`} style={{backgroundColor:c.hex}} onClick={()=>setDronePinColor(c.hex)}/>)}<input type="color" value={dronePinColor} onChange={e=>setDronePinColor(e.target.value)} style={{width:26,height:26,borderRadius:6,border:"1px solid rgba(255,255,255,0.1)",cursor:"pointer",padding:0,background:"none"}}/></div></div>
                  <div className="fg"><label className="fl">Size: {dronePinSize}px</label><div style={{display:"flex",alignItems:"center",gap:8}}><input type="range" min={80} max={400} value={dronePinSize} onChange={e=>setDronePinSize(Number(e.target.value))} style={{flex:1,accentColor:"var(--sa)"}}/><span style={{fontSize:11,color:"var(--std)",fontWeight:700,minWidth:28}}>{dronePinSize}</span></div></div>
                  <div className="fg"><label className="fl">Pin Label</label><input className="fi" value={dronePinText} onChange={e=>setDronePinText(e.target.value)} placeholder="e.g. 1500 m\u00b2 lot"/></div>
                  <div className="fg"><label className="fl">Label Color</label><div style={{display:"flex",gap:5}}>{[{hex:"#ffffff"},{hex:"#facc15"},{hex:"#000000"}].map(c=><div key={c.hex} className={`drone-swatch ${droneLabelColor===c.hex?"active":""}`} style={{backgroundColor:c.hex}} onClick={()=>setDroneLabelColor(c.hex)}/>)}</div></div>
                  <div className="fg"><label className="fl">Logo</label>{droneLogo?<div style={{display:"flex",alignItems:"center",gap:8}}><img src={droneLogo} alt="" style={{width:40,height:40,borderRadius:8,objectFit:"cover",border:"1px solid var(--sbr)"}}/><button className="sp" style={{flex:1}} onClick={()=>setDroneLogo(null)}>Remove</button></div>:<div onClick={()=>droneLogoRef.current?.click()} style={{border:"2px dashed var(--sbr)",borderRadius:10,padding:12,display:"flex",flexDirection:"column" as const,alignItems:"center",gap:4,cursor:"pointer"}}><Upload size={16} color="rgba(255,255,255,0.25)"/><span style={{fontSize:10,color:"var(--std)",fontWeight:600}}>Upload Logo</span></div>}<input ref={droneLogoRef} type="file" accept="image/*" style={{display:"none"}} onChange={droneLogoUpload}/></div>
                </Section>}
                {droneTool==="label"&&<Section title="Text Label" icon={Type}>
                  <div className="fg"><label className="fl">Text</label><input className="fi" value={droneLabelText} onChange={e=>setDroneLabelText(e.target.value)} placeholder="e.g. LOT #1 \u00b7 1,508m\u00b2"/></div>
                  <div className="fg"><label className="fl">Font Size: {droneLabelFontSize}px</label><div style={{display:"flex",alignItems:"center",gap:8}}><input type="range" min={14} max={80} value={droneLabelFontSize} onChange={e=>setDroneLabelFontSize(Number(e.target.value))} style={{flex:1,accentColor:"var(--sa)"}}/><span style={{fontSize:11,color:"var(--std)",fontWeight:700,minWidth:28}}>{droneLabelFontSize}</span></div></div>
                  <div className="fg"><label className="fl">Font</label><div style={{display:"flex",gap:3}}>{DRONE_FONT_OPTS.map(f=><button key={f.id} className={`drone-pill ${droneLabelFont===f.id?"active":""}`} onClick={()=>setDroneLabelFont(f.id)} style={{fontFamily:f.family}}>{f.label}</button>)}</div></div>
                  <div className="fg"><label className="fl">Color</label><div style={{display:"flex",flexWrap:"wrap" as const,gap:5}}>{DRONE_COLORS.map(c=><div key={c.hex} className={`drone-swatch ${droneLabelColor===c.hex?"active":""}`} style={{backgroundColor:c.hex}} onClick={()=>setDroneLabelColor(c.hex)}/>)}</div></div>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"4px 0"}}><div><span style={{fontSize:12,fontWeight:700,color:"var(--st)"}}>Background</span><span style={{fontSize:10,color:"var(--std)",display:"block",marginTop:1}}>Semi-transparent bg</span></div><button onClick={()=>setDroneLabelBg(!droneLabelBg)} style={{width:40,height:22,borderRadius:11,border:"none",cursor:"pointer",background:droneLabelBg?"var(--sa)":"rgba(255,255,255,0.12)",position:"relative"}}><div style={{width:16,height:16,borderRadius:"50%",background:"#fff",position:"absolute",top:3,left:droneLabelBg?21:3,transition:"left 0.2s",boxShadow:"0 1px 3px rgba(0,0,0,0.3)"}}/></button></div>
                </Section>}
                {droneTool==="select"&&!droneSelected&&<div style={{padding:"20px 0",textAlign:"center" as const}}><p style={{fontSize:11,color:"var(--std)",lineHeight:1.7}}>Click a shape on the canvas to select and edit it.</p></div>}
              </>}
            </div>
          </>}
          ...
          {isDroneMode&&leftPanel==="layers"&&<>
            ...
            <div style={{padding:"10px 14px"}}>
              ...
              :[...droneShapes].reverse().map(shape=>{
                ...
                return(<div key={shape.id} className={`drone-layer ${droneSelectedId===shape.id?"active":""}`} onClick={()=>{setDroneSelectedId(shape.id);setDroneTool("select");setLeftPanel("style");}}>
          ...
          {!isDroneMode&&<>
            ...
            {activeTab==="templates"&&leftPanel==="uploads"&&<><div className="ph"><Upload size={15} color="var(--sa)"/>Media</div><div style={{padding:14}}><UploadZone label="Listing Photo" imageUrl={listingPhoto} onUpload={f=>setListingPhoto(URL.createObjectURL(f))} onClear={()=>setListingPhoto(null)} uploading={false}/><div style={{marginTop:12}}><span className="fl">Stock Photos</span><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginTop:6}}>{DEMO_PHOTOS.map((url:string,i:number)=><div key={i} onClick={()=>setListingPhoto(url)} style={{aspectRatio:"1",borderRadius:8,overflow:"hidden",cursor:"pointer",border:listingPhoto===url?"2px solid var(--sa)":"1px solid var(--sbr)"}}><img src={url} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/></div>)}</div></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:14}}><UploadZone label="Headshot" imageUrl={headshot} onUpload={f=>setHeadshot(URL.createObjectURL(f))} onClear={()=>setHeadshot(null)} uploading={false} compact/><UploadZone label="Logo" imageUrl={logo} onUpload={f=>setLogo(URL.createObjectURL(f))} onClear={()=>setLogo(null)} uploading={false} compact/></div></div></>}
