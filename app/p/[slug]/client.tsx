...
interface PropertyWebsiteClientProps {
  property: any;
  agent: any;
          ...
          <div className="max-w-5xl mx-auto">
            {property.price && (
              <p className="text-white/90 text-2xl sm:text-4xl font-bold mb-2">
                ...
                {property.price_period && <span className="text-lg font-normal text-white/70">/{property.price_period}</span>}
              </p>
            )}
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {designExports.map((exp: any) => {
                ...
                if (dl?.includes("cloudinary.com")) {
                  if (isVideo) thumb = dl.replace("/video/upload/", "/video/upload/so_1,w_500,h_375,c_fill,f_jpg/").replace(/\.(mp4|mov|webm)$/i, ".jpg");
                  else thumb = dl.includes("/upload/") ? dl.replace("/upload/", "/upload/w_500,h_375,c_fill/") : dl;
                }
                const labels: Record<string, string> = { just_listed: "Just Listed", open_house: "Open House", price_reduced: "Price Reduced", just_sold: "Just Sold", yard_sign: "Yard Sign", property_pdf: "Property PDF", branding_card: "Branding Card" };
                return (
        ...
        <section className="py-16">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
              <h2 className={`text-2xl font-extrabold ${t.heading} mb-2 ${t.heroFont}`}>Interested in This Property?</h2>
              <p className={`${t.textMuted} text-sm mb-6`}>Send a message and {agentName || "the agent"} will get back to you.</p>
              <div className={`${t.cardBg} rounded-2xl border ${t.border} p-6`}>
                <ShowingRequestForm
