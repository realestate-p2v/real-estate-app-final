...
export default function ContentStudioPage() {
  ...
  const generateSocialPosts = async (video: ContentVideo) => {
    setGeneratingSocial(video.id);
    try {
      const narrations = (video.script || []).map((seg: any) => seg.narration).filter(Boolean);
      ...
      if (data.success && data.posts) {
        setSocialPosts(prev => ({ ...prev, [video.id]: data.posts }));
      }
    } catch (err) {
  ...
  const STYLE_LABELS: Record<string, { label: string; color: string }> = {
    bold: { label: "Bold / Controversial", color: "text-red-600 bg-red-50" },
    educational: { label: "Educational / Value", color: "text-blue-600 bg-blue-50" },
    relatable: { label: "Relatable / Story", color: "text-amber-600 bg-amber-50" },
  };
          ...
          {videos.length === 0 ? (
            ...
            <div className="space-y-4">
              {videos.map((video) => {
                ...
                const videoSocialPosts = socialPosts[video.id] || [];
                        ...
                        <div className="mt-4 pt-4 border-t border-border">
                          <div className="flex items-center justify-between mb-3">
                            ...
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() => generateSocialPosts(video)}
                              disabled={generatingSocial === video.id}
                              className="h-7 text-xs"
                            >
                              {generatingSocial === video.id ? (
                                <><Loader2 className="mr-1 h-3 w-3 animate-spin" /> Generating...</>
                              ) : videoSocialPosts.length > 0 ? (
                                <><RefreshCw className="mr-1 h-3 w-3" /> Regenerate</>
                              ) : (
                                <><Sparkles className="mr-1 h-3 w-3" /> Generate 3 Captions</>
                              )}
                            </Button>
                          </div>
                          ...
                          {videoSocialPosts.length > 0 && (
                            <div className="space-y-3">
                              {videoSocialPosts.map((post, pi) => {
                                const styleInfo = STYLE_LABELS[post.style] || { label: post.style, color: "text-gray-600 bg-gray-50" };
                                const postKey = `${video.id}-${pi}`;
                                return (
                                  <div key={pi} className="bg-muted/30 rounded-lg p-4">
                                    <div className="flex items-center justify-between mb-2">
                                      <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${styleInfo.color}`}>
                                        {styleInfo.label}
                                      </span>
                                      <button
                                        onClick={() => copyToClipboard(post.caption, postKey)}
